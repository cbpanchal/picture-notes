    ...
    onDrop(acceptedFiles, rejectedFiles) {
        ...
        if (filesToBeSent.length < this.state.printcount) {
            ...
            for (const i in filesToBeSent) {
                filesPreview.push(
                    <div key={i}>
                        <ImageWrapper src={filesToBeSent[i][0].preview} alt="image preview" />
                        {filesToBeSent[i][0].name}
                    </div>
                );
            }
            this.setState({ filesToBeSent, filesPreview, imageSelected: false, image: acceptedFiles[0] });
        }
        else {
...
export default connect(mapStateToProps, mapDispatchToProps)(PictureNotesContainer);
